...
inline void implicit_euler(Eigen::VectorXd &q, Eigen::VectorXd &qdot, double dt, 
    ...
    // lecture 5 slide 29 for gradient and hessian 
    // remeber the negative sign !!!
    auto g = [&](Eigen::VectorXd &gradient, Eigen::VectorXd v) {
        tmp_force.setZero();
        force(tmp_force, q + dt * v, v);
        gradient = mass * (v - qdot) - tmp_force * dt;
    };

    auto H = [&](Eigen::SparseMatrixd &hessian, Eigen::VectorXd v) {
    ...
    // iterate newton's method 5 times
